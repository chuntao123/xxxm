...
export function export_email_to_excel(th, jsonData, defaultTitle,value) {
  ...
  var file = new File([blob], "预测报表分析.xlsx", {type: 'application/octet-stream', lastModified: Date.now()});//blob转file
  ...
  request({
    ...
    if (res){
      modal.alertSuccess("导出成功，请查看您的邮箱！");
    }else {
      modal.alertError("导出失败，请稍后再试！");
    }
  })
}
